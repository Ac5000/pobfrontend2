...
[[maybe_unused]] static void dumpLuaStack(lua_State *L,
...
static int l_SetCallback(lua_State *L) {
  ...
  lua_pushvalue(L, 1);
  if (n >= 2) {
    LAssert(L, lua_isfunction(L, 2) || lua_isnil(L, 2),
...
static int l_SetMainObject(lua_State *L) {
  ...
  if (n >= 1) {
    LAssert(L, lua_istable(L, 1) || lua_isnil(L, 1),
...
static QFileInfoList *GetSearchHandle(lua_State *L, const char *method,
                                      bool valid) {
  LAssert(L, pobwindow->IsUserData(L, 1, "uisearchhandlemeta"),
          "searchHandle:%s() must be used on a search handle", method);
  auto searchHandle = (searchHandle_s *)lua_touserdata(L, 1);
  lua_remove(L, 1);
  if (valid) {
    LAssert(L, !searchHandle->fil->isEmpty(),
            "searchHandle:%s(): search handle is no longer valid (ran out of "
            "files to find)",
            method);
  }
...
static int l_Copy(lua_State *L) {
  ...
  LAssert(L, lua_isstring(L, 1), "Copy() argument 1: expected string, got %t",
          1);
...
static int l_LaunchSubScript(lua_State *L) {
  ...
  for (int i = 1; i <= 3; i++) {
    LAssert(L, lua_isstring(L, i),
            "LaunchSubScript() argument %d: expected string, got %t", i, i);
  }
  for (int i = 4; i <= n; i++) {
    LAssert(L,
            lua_isnil(L, i) || lua_isboolean(L, i) || lua_isnumber(L, i) ||
                lua_isstring(L, i),
            "LaunchSubScript() argument %d: only nil, boolean, number and "
            "string types can be passed to sub script",
            i);
  }
  ...
  pobwindow->connect(pobwindow->subScriptList[slot].get(), &SubScript::finished,
                     pobwindow, &POBWindow::subScriptFinished);
...
static int l_AbortSubScript(lua_State *L) {
  ...
  /*
    ...
    LAssert(L, lua_islightuserdata(L, 1), "AbortSubScript() argument 1: expected
    subscript ID, got %t", 1); notdword slot = (notdword)lua_touserdata(L, 1);
    LAssert(L, slot < pobwindow->subScriptSize &&
    pobwindow->subScriptList[slot], "AbortSubScript() argument 1: invalid
    subscript ID"); LAssert(L, pobwindow->subScriptList[slot]->IsRunning(),
...
static int l_IsSubScriptRunning(lua_State *L) {
  ...
  /*
    ...
    LAssert(L, lua_islightuserdata(L, 1), "IsSubScriptRunning() argument 1:
    expected subscript ID, got %t", 1); notdword slot =
    (notdword)lua_touserdata(L, 1); LAssert(L, slot < pobwindow->subScriptSize
    && pobwindow->subScriptList[slot], "IsSubScriptRunning() argument 1: invalid
    subscript ID"); lua_pushboolean(L,
    pobwindow->subScriptList[slot]->IsRunning()); return 1;
  */
}
