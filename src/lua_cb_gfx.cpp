...
imgHandle_s *GetImgHandle(lua_State *L, const char *method) {
  LAssert(L, pobwindow->IsUserData(L, 1, "uiimghandlemeta"),
...
int l_RenderInit(lua_State *L) { return 0; }
...
int l_SetClearColor(lua_State *L) {
  ...
  for (int i = 1; i <= 3; i++) {
    LAssert(L, lua_isnumber(L, i),
            "SetClearColor() argument %d: expected number, got %t", i, i);
    color[i - 1] = (float)lua_tonumber(L, i);
  }
  if (n >= 4 && !lua_isnil(L, 4)) {
    LAssert(L, lua_isnumber(L, 4),
...
int l_SetDrawLayer(lua_State *L) {
  ...
  LAssert(L, lua_isnumber(L, 1) || lua_isnil(L, 1),
          "SetDrawLayer() argument 1: expected number or nil, got %t", 1);
  if (n >= 2) {
    LAssert(L, lua_isnumber(L, 2),
...
int l_SetViewport(lua_State *L) {
  ...
  if (n) {
    ...
    for (int i = 1; i <= 4; i++) {
      LAssert(L, lua_isnumber(L, i),
              "SetViewport() argument %d: expected number, got %t", i, i);
    }
    pobwindow->AppendCmd(std::make_unique<ViewportCmd>(
        (int)lua_tointeger(L, 1), (int)lua_tointeger(L, 2),
        (int)lua_tointeger(L, 3), (int)lua_tointeger(L, 4)));
  } else {
    pobwindow->AppendCmd(std::make_unique<ViewportCmd>(0, 0, pobwindow->width,
                                                       pobwindow->height));
  }
  return 0;
}
...
int l_SetDrawColor(lua_State *L) {
  ...
  int n = lua_gettop(L);
  LAssert(L, n >= 1,
          "Usage: SetDrawColor(red, green, blue[, alpha]) or "
          "SetDrawColor(escapeStr)");
  float color[4];
  if (lua_type(L, 1) == LUA_TSTRING) {
    LAssert(L, IsColorEscape(lua_tostring(L, 1)),
    ...
    for (int i = 1; i <= 3; i++) {
      LAssert(L, lua_isnumber(L, i),
              "SetDrawColor() argument %d: expected number, got %t", i, i);
      color[i - 1] = (float)lua_tonumber(L, i);
    }
    if (n >= 4 && !lua_isnil(L, 4)) {
      LAssert(L, lua_isnumber(L, 4),
...
int l_DrawImage(lua_State *L) {
  ...
  int n = lua_gettop(L);
  LAssert(L, n >= 5,
          "Usage: DrawImage({imgHandle|nil}, left, top, width, height[, "
          "tcLeft, tcTop, tcRight, tcBottom])");
  LAssert(L, lua_isnil(L, 1) || pobwindow->IsUserData(L, 1, "uiimghandlemeta"),
  ...
  if (n > 5) {
    ...
    pobwindow->AppendCmd(
        std::make_unique<DrawImageCmd>(tex_idx, arg[0], arg[1], arg[2], arg[3],
    ...
    pobwindow->AppendCmd(std::make_unique<DrawImageCmd>(tex_idx, arg[0], arg[1],
                                                        arg[2], arg[3]));
  }
  return 0;
}
...
void DrawImageQuadCmd::BindTexture() { pobwindow->GetTexture(tex).bind(); }

int l_DrawImageQuad(lua_State *L) {
  LAssert(L, pobwindow->isDrawing, "DrawImageQuad() called outside of OnFrame");
  int n = lua_gettop(L);
  LAssert(L, n >= 9,
          "Usage: DrawImageQuad({imgHandle|nil}, x1, y1, x2, y2, x3, y3, x4, "
          "y4[, s1, t1, s2, t2, s3, t3, s4, t4])");
  LAssert(L, lua_isnil(L, 1) || pobwindow->IsUserData(L, 1, "uiimghandlemeta"),
          "DrawImageQuad() argument 1: expected image handle or nil, got %t",
          1);
  TextureIndex tex_idx = 0;
  if (!lua_isnil(L, 1)) {
    auto imgHandle = (imgHandle_s *)lua_touserdata(L, 1);
  ...
  if (n > 9) {
    ...
    pobwindow->AppendCmd(std::make_unique<DrawImageQuadCmd>(
        tex_idx, arg[0], arg[1], arg[2], arg[3], arg[4], arg[5], arg[6],
        arg[7]));
  }
  return 0;
}
...
DrawStringCmd::DrawStringCmd(float X, float Y, int Align, int Size, int Font,
  ...
  QString cacheKey =
      (QString::number(Font) + "_" + QString::number(Size) + "_" + text);
  if (pobwindow->stringCache.contains(cacheKey)) {
    ...
    pobwindow->stringCache.insert(cacheKey,
                                  new std::shared_ptr<QOpenGLTexture>(tex));
  }
  int width = 0;
  int height = 0;
  if (tex.get() != nullptr) {
...
int l_DrawStringWidth(lua_State *L) {
  ...
  LAssert(L, lua_isnumber(L, 1),
          "DrawStringWidth() argument 1: expected number, got %t", 1);
  LAssert(L, lua_isstring(L, 2),
          "DrawStringWidth() argument 2: expected string, got %t", 2);
  LAssert(L, lua_isstring(L, 3),
          "DrawStringWidth() argument 3: expected string, got %t", 3);
  ...
  if (pobwindow->stringCache.contains(cacheKey) &&
      pobwindow->stringCache[cacheKey]->get()) {
...
int l_DrawStringCursorIndex(lua_State *L) {
  ...
  int yidx =
      std::max(0, std::min((int)texts.size() - 1, curY / fm.lineSpacing()));
  text = texts[yidx];
  int i = 0;
  for (; i <= text.size(); i++) {
