...
void TextureLoader::collect_loaded_textures(
    std::vector<std::pair<TextureIndex, std::unique_ptr<QImage>>> &loaded) {
...
// TODO - This is now broken with PoB2 because they started using .dds files for
// the textures. (/src/TreeData/*) Also, the .dds files are compressed using
// .zst. QT6 no longer includes dds plugin by default either...
// See https://forum.qt.io/topic/153202/dds-image-format-plugin for how to
// include it. But maybe figure out a different way to do this...
void TextureLoader::run() {
  while (_loop) {
    ...
    for (auto e = end(_to_load_th); iter != e; ++iter) {
      ...
      auto &loaded_tex =
          _loaded_th.emplace_back(std::make_pair(llt->index, nullptr));
