...
// Font alignment
enum r_fontAlign_e { F_LEFT, F_CENTRE, F_RIGHT, F_CENTRE_X, F_RIGHT_X };
...
public:
  ...
  DrawTextureCmd(float X0, float Y0, float X1, float Y1, float X2, float Y2,
                 float X3, float Y3, float S0 = 0, float T0 = 0, float S1 = 1,
                 float T1 = 0, float S2 = 1, float T2 = 1, float S3 = 0,
                 float T3 = 1)
      : x{X0, X1, X2, X3}, y{Y0, Y1, Y2, Y3}, s{S0, S1, S2, S3},
        t{T0, T1, T2, T3} {}
...
public:
  DrawImageQuadCmd() {}
  DrawImageQuadCmd(TextureIndex Tex, float X0, float Y0, float X1, float Y1,
                   ...
                   float S3 = 0, // arg[16]
                   float T3 = 1) // arg[17]
      : DrawTextureCmd(X0, Y0, X1, Y1, X2, Y2, X3, Y3, S0, T0, S1, T1, S2, T2,
                       S3, T3),
        tex(Tex) {}
...
public:
  DrawImageCmd(TextureIndex tex, float x, float y, float w, float h,
               float s1 = 0, float t1 = 0, float s2 = 1.0f, float t2 = 1.0f)
      : DrawImageQuadCmd(tex, x, y, x + w, y, x + w, y + h, x, y + h, s1, t1,
                         s2, t1, s2, t2, s1, t2) {}
};

class DrawStringCmd : public DrawTextureCmd {
public:
  DrawStringCmd(float X, float Y, int Align, int Size, int Font,
                const char *Text);
  ~DrawStringCmd() {}
